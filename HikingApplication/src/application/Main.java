...
public class Main extends Application {
	...
	private Scene scene;
	private String userProfile = System.getProperty("user.name");
	...
	public void start(Stage stage) throws Exception {
		...
		// Auto-Rotate photo based on image Metadata
		...
		window.show();
		// image delay
		Timeline timeline = new Timeline(new KeyFrame(Duration.millis(3000), ae -> createPaths(window)));
		timeline.play();
	}
	...
	public void createPaths(Stage window) {
		...
		if (!homeFolder.exists() && !homeFolder.isFile()) {
			...
			if (Table.hikeData.size() == 0) {
				...
				t.deleteButton.setDisable(false);
			}
			new HelperFunctions().filterConversions(Table.table, t.totalMilesLabel, t.totalHoursLabel, t.totalDaysLabel,
					t.totalCountLabel);
	...
	public void createImageFolder() throws IOException {
		...
		if (!imageFolder.exists() && !imageFolder.isFile()) {
			...
			File outputFile = new File(
					"C:\\Users\\" + System.getProperty("user.name") + "\\JustHike\\Load_Screen_Images\\Images");
